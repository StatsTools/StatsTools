...
pkgs <- c("StatsTLF", "StatsConsort", "StatsSummary", "StatsSample")

StatsTools_attach <- function() {
  # Create `to_load` which is a character vector of all StatsTools
  ...
  # StatsTools with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("StatsTools ", package_version("StatsTools"))
  )

  # Return a character string containing the package version for each of StatsTools's constituents
...
StatsTools_detach <- function() {
  pak <- paste0("package:", c(pkgs, "StatsTools"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by StatsTools
#'
#' @export
#'
#' @examples
#' StatsTools_packages()
StatsTools_packages <- function() {
  # get all imports from StatsTools's package description file
  raw <- utils::packageDescription("StatsTools")$Imports
